...
class App:
    def __init__(self, width=640, height=400, fps=30):
        ...
        self.upper_letters = self.spellathon.letters.upper()
        self.central_letter = self.upper_letters[self.spellathon.central_index]
        self.upper_letters = self.upper_letters[:self.spellathon.central_index] + \
            self.upper_letters[self.spellathon.central_index+1:]
    ...
    def on_render(self):
        self.draw_polygon((0.9 * self.height)/2, 6,
                          (255, 204, 255), self.width/2, self.height/2)
        self.draw_polygon((0.4 * self.height)/2, 6,
                          (255, 204, 204), self.width/2, self.height/2)
        ...
        for i in range(n):
            ...
            self.draw_alpha(self.upper_letters[i].upper(), px, py)
        self.draw_alpha(self.spellathon.get_central_char(
        ).upper(), self.width/2, self.height/2)
